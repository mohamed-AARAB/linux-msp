...
void resetMspPort(mspPort_t *mspPortToReset, UART_device *serialPort) {
  bool isMWC = mspPortToReset->isMWC;

  memset(mspPortToReset, 0, sizeof(mspPort_t));

  mspPortToReset->isMWC = isMWC;

  mspPortToReset->port = serialPort;
}

void mspSerialAllocatePort(mspPort_t *mspPort, UART_device *serialPort,
                           bool isMWC) {
  if (serialPort) {
...
void mspSerialReleasePortIfAllocated(mspPort_t *mspPort) {
  if (mspPort->port) {
    uart_end(mspPort->port);

    bool isMWC = mspPort->isMWC;
...
static bool mspSerialProcessReceivedData(mspPort_t *mspPort, uint8_t c) {
  ...
  case MSP_HEADER_START: // Waiting for 'M' (MSPv1 / MSPv2_over_v1) or 'X'
                         // (MSPv2 native)
    switch (c) {
    case 'M':
  ...
  case MSP_HEADER_V1: // Now receive v1 header (size/cmd), this is already
                      // checksummable
    ...
    if (mspPort->offset == sizeof(mspHeaderV1_t)) {
      ...
      if (hdr->size > MSP_PORT_INBUF_SIZE) {
        ...
        mspPort->c_state =
            mspPort->dataSize > 0
                ? MSP_PAYLOAD_V1
                : MSP_CHECKSUM_V1; // If no payload - jump to checksum byte
      }
    }
    break;
  ...
  case MSP_HEADER_V2_OVER_V1: // V2 header is part of V1 payload - we need to
    ...
    if (mspPort->offset == (sizeof(mspHeaderV2_t) + sizeof(mspHeaderV1_t))) {
      ...
      if (hdrv2->size > MSP_PORT_INBUF_SIZE) {
        ...
        mspPort->offset = 0; // re-use buffer
        mspPort->c_state = mspPort->dataSize > 0 ? MSP_PAYLOAD_V2_OVER_V1
  ...
  case MSP_CHECKSUM_V2_OVER_V1:
    ...
    if (mspPort->checksum2 == c) {
      mspPort->c_state =
          MSP_CHECKSUM_V1; // Checksum 2 correct - verify v1 checksum
    } else {
      mspPort->c_state = MSP_IDLE;
    }
    break;
  ...
  case MSP_HEADER_V2_NATIVE:
    ...
    if (mspPort->offset == sizeof(mspHeaderV2_t)) {
      ...
      if (hdrv2->size > MSP_PORT_INBUF_SIZE) {
        ...
        mspPort->cmdFlags = hdrv2->flags;
        mspPort->offset = 0; // re-use buffer
        mspPort->c_state = mspPort->dataSize > 0 ? MSP_PAYLOAD_V2_NATIVE
...
static uint8_t mspSerialChecksumBuf(uint8_t checksum, const uint8_t *data,
                                    int len) {
  while (len-- > 0) {
    checksum ^= *data++;
  }
...
static int mspSerialSendFrame(mspPort_t *msp, const uint8_t *hdr, int hdrLen,
  ...
  UART_device *port = msp->port;
  if (!port) {
    return 0;
  }
  // VSP MSP port might be unconnected. To prevent blocking - check if it's
  // connected first
  ...
  //  a) TX buffer is completely empty (we are talking to well-behaving party
  //  that follows request-response scheduling;
  ...
  /*    ((int)serialTxBytesFree(port) < totalFrameLength))*/
  /*  return 0;*/

  // Transmit frame
  uart_write(port, hdr, hdrLen);
  uart_write(port, data, dataLen);
  uart_write(port, crc, crcLen);
...
static int mspSerialEncode(mspPort_t *msp, mspPacket_t *packet,
  ...
  uint8_t hdrBuf[16] = {
      '$', mspMagic[mspVersion],
      packet->result == MSP_RESULT_ERROR ? '!' : (msp->isMWC ? '>' : '<')};
  uint8_t crcBuf[2];
  int hdrLen = 3;
  int crcLen = 0;
...
#define V1_CHECKSUM_STARTPOS 3
  if (mspVersion == MSP_V1) {
    ...
    // Pre-calculate CRC
    crcBuf[crcLen] = mspSerialChecksumBuf(0, hdrBuf + V1_CHECKSUM_STARTPOS,
                                          hdrLen - V1_CHECKSUM_STARTPOS);
    crcBuf[crcLen] =
        mspSerialChecksumBuf(crcBuf[crcLen], sbufPtr(&packet->buf), dataLen);
    crcLen++;
    ...
    const int v1PayloadSize = sizeof(mspHeaderV2_t) + dataLen +
    ...
    crcBuf[crcLen] =
        crc8_dvb_s2_update(0, (uint8_t *)hdrV2, sizeof(mspHeaderV2_t));
    crcBuf[crcLen] =
        crc8_dvb_s2_update(crcBuf[crcLen], sbufPtr(&packet->buf), dataLen);
    crcLen++;
  ...
  // Send the frame
  return mspSerialSendFrame(msp, hdrBuf, hdrLen, sbufPtr(&packet->buf), dataLen,
                            crcBuf, crcLen);
}

static mspPostProcessFnPtr
mspSerialProcessReceivedCommand(mspPort_t *msp,
  ...
  const mspResult_e status =
      mspProcessCommandFn(&command, &reply, &mspPostProcessFn);
...
static void mspProcessPendingRequest(mspPort_t *mspPort) {
  ...
  if ((mspPort->pendingRequest == MSP_PENDING_NONE) ||
      (millis() - mspPort->lastActivityMs < 100)) {
    return;
  }
  ...
  switch (mspPort->pendingRequest) {
    ...
    /*  if (!cliMode) {*/
    /*    // When we enter CLI mode - disable this MSP port. Don't care about*/
    /*    // preserving the port since CLI can only be exited via reboot*/
...
void mspSerialProcessPort(mspPort_t *const mspPort,
  ...
  if (uart_RxBytesWaiting(mspPort->port) > 0) {
    ...
    if (mspPostProcessFn) {
      uart_waitTxToFinish(mspPort->port);
...
/*    if (mspPort->port) {*/
/*      mspSerialProcessOnePort(mspPort, evaluateNonMspData,
 * mspProcessCommandFn);*/
/*    }*/
/*  }*/
/*}*/

void mspSerialInit(mspPort_t *mspPort, UART_device *serialPort, bool isMWC) {
  /*memset(mspPorts, 0, sizeof(mspPorts));*/
  memset(mspPort, 0, sizeof(mspPort_t));
  mspSerialAllocatePort(mspPort, serialPort, isMWC);
}

int mspSerialPushPort(mspPort_t *mspPort, uint16_t cmd, const uint8_t *data,
                      int datalen, mspVersion_e version) {
...
/*int mspSerialPushVersion(uint8_t cmd, const uint8_t *data, int datalen,*/
...
int mspSerialPush(mspPort_t *mspPort, uint8_t cmd, const uint8_t *data,
                  int datalen) {
  return mspSerialPushPort(mspPort, cmd, data, datalen, MSP_V2_NATIVE);
}

/*uint32_t mspSerialTxBytesFree(UART_device *port) {*/
/*  return serialTxBytesFree(port);*/
/*}*/

/*mspPort_t *mspSerialPortFind(const UART_device *serialPort) {*/
/*  for (int portIndex = 0; portIndex < MAX_MSP_PORT_COUNT; portIndex++) {*/
